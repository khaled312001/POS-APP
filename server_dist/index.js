...
__export(schema_exports, {
  ...
  branches: () => branches,
  broadcastOrderRecipients: () => broadcastOrderRecipients,
  broadcastOrders: () => broadcastOrders,
  calls: () => calls,
  ...
  insertBroadcastOrderRecipientSchema: () => insertBroadcastOrderRecipientSchema,
  insertBroadcastOrderSchema: () => insertBroadcastOrderSchema,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    chatRooms = (0, import_mysql_core.mysqlTable)("chat_rooms", {
      id: (0, import_mysql_core.serial)("id").primaryKey(),
      orderId: (0, import_mysql_core.int)("order_id").references(() => onlineOrders.id, { onDelete: "cascade" }).notNull().unique(),
      tenantId: (0, import_mysql_core.int)("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
      customerId: (0, import_mysql_core.int)("customer_id").references(() => customers.id, { onDelete: "set null" }),
      customerName: (0, import_mysql_core.text)("customer_name"),
      customerPhone: (0, import_mysql_core.text)("customer_phone"),
      status: (0, import_mysql_core.text)("status").notNull().default("open"),
      // open | closed
      lastMessageAt: (0, import_mysql_core.timestamp)("last_message_at"),
      unreadByCustomer: (0, import_mysql_core.int)("unread_by_customer").default(0),
      unreadByTenant: (0, import_mysql_core.int)("unread_by_tenant").default(0),
      createdAt: (0, import_mysql_core.timestamp)("created_at").defaultNow()
    });
    chatMessages = (0, import_mysql_core.mysqlTable)("chat_messages", {
      id: (0, import_mysql_core.serial)("id").primaryKey(),
      roomId: (0, import_mysql_core.int)("room_id").references(() => chatRooms.id, { onDelete: "cascade" }).notNull(),
      senderType: (0, import_mysql_core.text)("sender_type").notNull(),
      // customer | tenant | driver | system
      senderId: (0, import_mysql_core.int)("sender_id"),
      // customer.id, employee.id, vehicle.id (nullable for system)
      senderName: (0, import_mysql_core.text)("sender_name"),
      body: (0, import_mysql_core.text)("body").notNull(),
      attachmentUrl: (0, import_mysql_core.text)("attachment_url"),
      readByOther: (0, import_mysql_core.boolean)("read_by_other").default(false),
      createdAt: (0, import_mysql_core.timestamp)("created_at").defaultNow()
    });
    broadcastOrders = (0, import_mysql_core.mysqlTable)("broadcast_orders", {
      id: (0, import_mysql_core.serial)("id").primaryKey(),
      broadcastToken: (0, import_mysql_core.varchar)("broadcast_token", { length: 64 }).notNull().unique(),
      customerName: (0, import_mysql_core.text)("customer_name").notNull(),
      customerPhone: (0, import_mysql_core.text)("customer_phone").notNull(),
      customerEmail: (0, import_mysql_core.text)("customer_email"),
      customerAddress: (0, import_mysql_core.text)("customer_address"),
      ...
      items: (0, import_mysql_core.json)("items").$type().notNull().default([]),
      notes: (0, import_mysql_core.text)("notes"),
      estimatedTotal: (0, import_mysql_core.decimal)("estimated_total", { precision: 10, scale: 2 }).default("0"),
      paymentMethod: (0, import_mysql_core.text)("payment_method").notNull().default("cash"),
      // cash, card
      status: (0, import_mysql_core.text)("status").notNull().default("pending"),
      // pending, claimed, expired, cancelled
      claimedByTenantId: (0, import_mysql_core.int)("claimed_by_tenant_id").references(() => tenants.id, { onDelete: "set null" }),
      claimedAt: (0, import_mysql_core.timestamp)("claimed_at"),
      onlineOrderId: (0, import_mysql_core.int)("online_order_id"),
      // set once claimed — links to the normal online_orders row
      expiresAt: (0, import_mysql_core.timestamp)("expires_at").notNull(),
      cancelledReason: (0, import_mysql_core.text)("cancelled_reason"),
      createdAt: (0, import_mysql_core.timestamp)("created_at").defaultNow(),
      updatedAt: (0, import_mysql_core.timestamp)("updated_at").defaultNow()
    });
    broadcastOrderRecipients = (0, import_mysql_core.mysqlTable)("broadcast_order_recipients", {
      id: (0, import_mysql_core.serial)("id").primaryKey(),
      broadcastOrderId: (0, import_mysql_core.int)("broadcast_order_id").references(() => broadcastOrders.id, { onDelete: "cascade" }).notNull(),
      tenantId: (0, import_mysql_core.int)("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
      response: (0, import_mysql_core.text)("response"),
      // null = pending, "rejected" | "accepted" | "missed"
      respondedAt: (0, import_mysql_core.timestamp)("responded_at"),
      createdAt: (0, import_mysql_core.timestamp)("created_at").defaultNow()
    });
    ...
    insertChatRoomSchema = (0, import_drizzle_zod.createInsertSchema)(chatRooms).omit({ id: true, createdAt: true, lastMessageAt: true, unreadByCustomer: true, unreadByTenant: true });
    insertChatMessageSchema = (0, import_drizzle_zod.createInsertSchema)(chatMessages).omit({ id: true, createdAt: true, readByOther: true });
    insertBroadcastOrderSchema = (0, import_drizzle_zod.createInsertSchema)(broadcastOrders).omit({ id: true, createdAt: true, updatedAt: true, claimedAt: true, claimedByTenantId: true, onlineOrderId: true });
    insertBroadcastOrderRecipientSchema = (0, import_drizzle_zod.createInsertSchema)(broadcastOrderRecipients).omit({ id: true, createdAt: true });
  }
});
...
`);
      ...
      async closeShift(id, data) {
        ...
        const [shift] = await db.select().from(shifts).where((0, import_drizzle_orm.eq)(shifts.id, id)).limit(1);
        return shift;
      },
      ...
      async getProductBatches(productId, tenantId) {
        ...
        const { and: and7 } = await import("drizzle-orm");
        return db.select().from(productBatches).where(and7(...conditions)).orderBy(productBatches.expiryDate);
      },
      ...
      async createVehicle(data) {
        ...
        if (!payload.driverAccessToken && (payload.driverName || payload.driverPhone)) {
          ...
          payload.driverAccessToken = randomBytes4(24).toString("hex");
        }
        ...
        for (const [tenantId, phones] of tenantPhoneGroups.entries()) {
          ...
          const existingCustomers = await db.select().from(customers).where(
            (0, import_drizzle_orm.and)(
              ...
              inArray2(customers.phone, phones)
            )
          );
      ...
      async getActiveDrivers(tenantId) {
        const rows = await db.select().from(vehicles).where(
          (0, import_drizzle_orm.and)(
            ...
            import_drizzle_orm.sql`${vehicles.driverName} IS NOT NULL AND ${vehicles.driverName} != ''`
          )
        );
        const { randomBytes: randomBytes4 } = await import("crypto");
        for (const r of rows) {
  ...
  app2.get("/api/fix-schema-and-seed", async (_req, res) => {
    try {
      ...
      for (const table of tables2) {
        try {
          await db2.execute(sql7.raw(`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS tenant_id integer`));
      ...
      for (const table of tables2) {
        try {
          const r = await db2.execute(
            sql7.raw(`UPDATE \`${table}\` SET tenant_id = ${tid} WHERE tenant_id IS NULL`)
          );
...
Address: ${customerAddress || "Pickup"}`
  ...
  const driverTokenFromRequest = (req) => {
    const bearer = (req.headers.authorization || "").replace(/^Bearer\s+/i, "").trim();
    return (bearer || req.body && req.body.token || req.query && req.query.token || "").trim();
  };
...
Leave a quick review: ${process.env.APP_URL || ""}/track/${order.trackingToken}#rate`
  ...
  app2.post("/api/delivery/driver/status", async (req, res) => {
    try {
      const token = driverTokenFromRequest(req);
      if (!token) return res.status(401).json({ error: "Driver token required" });
      const driver = await storage.getVehicleByAccessToken(token);
  ...
  app2.post("/api/delivery/driver/location", async (req, res) => {
    try {
      ...
      const oid = orderId ? Number(orderId) : void 0;
...
function registerBroadcastRoutes(app2) {
  ...
  app2.get("/api/delivery/broadcast/menu", async (_req, res) => {
    try {
  ...
  app2.post("/api/delivery/broadcast", async (req, res) => {
    ...
    try {
      ...
      const expiresAt = new Date(Date.now() + BROADCAST_TTL_MINUTES * 60 * 1e3);
      const [inserted] = await db.insert(broadcastOrders).values({
        ...
        items,
        notes: notes ? String(notes).slice(0, 2e3) : null,
      ...
      try {
        callerIdService.broadcast(payload);
      } catch (_) {
      }
      try {
        pushService.broadcast({ type: "broadcast_new", title: "\u{1F6F5} New Broadcast Order", body: `${row.customerName} \u2014 CHF ${row.estimatedTotal || "?"}`, data: { id: row.id, token: row.broadcastToken } });
      } catch (_) {
      }
      res.status(201).json({
        id: row.id,
  ...
  app2.get("/api/broadcast-orders/pending", async (req, res) => {
    try {
      const tenantId = Number(req.query.tenantId);
      if (!tenantId) return res.status(400).json({ error: "tenantId required" });
      const now = /* @__PURE__ */ new Date();
  ...
  app2.post("/api/broadcast-orders/:id/accept", async (req, res) => {
    try {
      const id = Number(req.params.id);
      ...
      const [ooInsert] = await db.insert(onlineOrders).values({
        ...
        paymentStatus: "pending",
        status: "accepted",
        // already accepted by the tenant by virtue of claiming
        orderType: "delivery",
        notes: bc.notes || null,
      ...
      try {
        callerIdService.broadcast(claimPayload);
      } catch (_) {
      }
      try {
        callerIdService.broadcast({ type: "new_online_order", tenantId, orderId: onlineOrderId, orderNumber }, tenantId);
      } catch (_) {
      }
      try {
        pushService.notifyNewOrder(orderNumber, subtotal || bc.estimatedTotal || "0", tenantId);
      } catch (_) {
      }
      res.json({
        success: true,
  ...
  app2.post("/api/broadcast-orders/:id/reject", async (req, res) => {
    try {
      const id = Number(req.params.id);
      const tenantId = Number(req.body?.tenantId || req.tenantId);
      if (!id || !tenantId) return res.status(400).json({ error: "id + tenantId required" });
...
function registerCustomerExtraRoutes(app2) {
  ...
  app2.post("/api/delivery/auth/guest", async (req, res) => {
    try {
      ...
      if (!customer.hasAccount && guestName && customer.name !== guestName) {
        try {
          await storage.updateCustomer(customer.id, { name: guestName });
        } catch {
        }
      }
      const token = await createCustomerSession(customer.id, tid, req.headers["user-agent"]);
      res.json({
        ...
        customer: {
          id: customer.id,
          name: guestName,
          // honor the user-provided name in the response
  ...
  app2.get("/api/customer/chats", async (req, res) => {
    try {
      await ensureChatTables();
      const customer = await getAuthenticatedCustomer(req.headers.authorization);
      if (!customer) return res.status(401).json({ error: "Not authenticated" });
  ...
  app2.get("/api/customer/chats/order/:orderId", async (req, res) => {
    try {
      await ensureChatTables();
  ...
  app2.post("/api/customer/chats/order/:orderId/messages", async (req, res) => {
    try {
      await ensureChatTables();
      const orderId = Number(req.params.orderId);
      const { body } = req.body || {};
      ...
      try {
        callerIdService.broadcast(payload, room.tenantId);
      } catch {
      }
      res.json({ success: true });
    } catch (e) {
  ...
  app2.post("/api/chat/order/:orderId/ensure", async (req, res) => {
    try {
      const orderId = Number(req.params.orderId);
  ...
  app2.post("/api/chat/rooms/:id/messages", async (req, res) => {
    try {
      await ensureChatTables();
      const id = Number(req.params.id);
...
var PUBLIC_ROUTES = [
  ...
  "/api/delivery/driver/",
  // All driver endpoints (auth via Bearer driver token)
  "/api/delivery/search",
  ...
  "/api/delivery/broadcast",
  // Public broadcast order creation (drop-shipping)
  "/api/delivery/broadcast/menu",
  // Public aggregated menu for /broadcast page
  "/api/customer/chats",
  // Customer chat list (auth via Bearer customer token)
  "/api/customer/chats/",
...
function configureExpoAndLanding(app2) {
  ...
  app2.use(async (req, res, next) => {
    ...
    }
    {
      ...
      if (m) {
        try {
          const sub = m[1] || "";
          if (sub === "/app.js") {
            const jsPath = path4.resolve(process.cwd(), "delivery-app", "customer.app.js");
            if (!fs5.existsSync(jsPath)) return res.status(404).end();
            res.setHeader("Content-Type", "application/javascript; charset=utf-8");
            res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
            return res.status(200).send(fs5.readFileSync(jsPath, "utf-8"));
          }
          if (sub === "/manifest.json") {
            ...
            return res.status(200).json({
              ...
              icons: [
                { src: "/api/delivery-app/icons/icon-192.png", sizes: "192x192", type: "image/png" },
                { src: "/api/delivery-app/icons/icon-512.png", sizes: "512x512", type: "image/png" }
              ]
            });
          }
          const shellPath = path4.resolve(process.cwd(), "delivery-app", "customer.html");
          if (!fs5.existsSync(shellPath)) return res.status(503).send("<h1>Customer app not deployed</h1>");
          res.setHeader("Content-Type", "text/html; charset=utf-8");
          res.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
          return res.status(200).send(fs5.readFileSync(shellPath, "utf-8"));
        } catch (err) {
    ...
    if (req.path === "/broadcast" || req.path === "/broadcast/" || req.path === "/api/broadcast" || req.path === "/api/broadcast/") {
      try {
        const bcPath = path4.resolve(process.cwd(), "delivery-app", "broadcast.html");
        if (!fs5.existsSync(bcPath)) {
        ...
        let html = fs5.readFileSync(bcPath, "utf-8");
        if (basePath) {
    ...
    if (deliveryMatch) {
      try {
        ...
        config = await storage2.getLandingPageConfigBySlug(isBrandAlias ? "pizza-lemon" : slug);
        if (!config) {
          config = await storage2.getLandingPageConfigBySlug("pizza-lemon");
          if (config) {
            const displayName = slug.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
            config.storeName = displayName;
            config.heroTitle = displayName;
        ...
        const configJson = JSON.stringify({
          ...
          primaryColor: config.primaryColor || "#FF5722",
          accentColor: config.accentColor || "#2FD3C6",
          currency: tenant.currency || process.env.DEFAULT_CURRENCY || "CHF",
          language: config.language || "en",
          storeName: config.storeName || config.heroTitle || config.name || tenant.businessName,
    ...
    if (driverMatch) {
      ...
      let html = fs5.readFileSync(driverIndexPath, "utf-8");
      const isApiPrefixed = req.path.startsWith("/api/");
      if (isApiPrefixed) {
...
async function initStripe() {
  ...
  try {
    ...
    try {
      ...
      log2("Stripe migrations skipped (stripe-replit-sync not available):", migErr?.message);
    }
...
(async () => {
  ...
  try {
    const { db: db2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const { sql: sql7 } = await import("drizzle-orm");
    const tenantsResult = await db2.execute(sql7`SELECT id FROM tenants LIMIT 1`);
    ...
    if (tenantRows && tenantRows.length > 0) {
      ...
      for (const table of tenantTables) {
        try {
          const r = await db2.execute(sql7.raw(`UPDATE \`${table}\` SET tenant_id = ${tid} WHERE tenant_id IS NULL`));
