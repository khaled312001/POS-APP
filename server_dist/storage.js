...
var kitchenOrders = pgTable("kitchen_orders", {
  id: serial("id").primaryKey(),
  saleId: integer("sale_id").references(() => sales.id, { onDelete: "cascade" }).notNull(),
  branchId: integer("branch_id").references(() => branches.id, { onDelete: "cascade" }),
  tableNumber: text("table_number"),
...
var cashDrawerOperations = pgTable("cash_drawer_operations", {
  id: serial("id").primaryKey(),
...
var warehouseTransfers = pgTable("warehouse_transfers", {
  id: serial("id").primaryKey(),
...
var productBatches = pgTable("product_batches", {
  id: serial("id").primaryKey(),
  productId: integer("product_id").references(() => products.id, { onDelete: "cascade" }).notNull(),
...
var stockCounts = pgTable("stock_counts", {
  id: serial("id").primaryKey(),
  branchId: integer("branch_id").references(() => branches.id, { onDelete: "cascade" }).notNull(),
  employeeId: integer("employee_id").references(() => employees.id, { onDelete: "cascade" }).notNull(),
...
var stockCountItems = pgTable("stock_count_items", {
  id: serial("id").primaryKey(),
...
var supplierContracts = pgTable("supplier_contracts", {
  id: serial("id").primaryKey(),
  supplierId: integer("supplier_id").references(() => suppliers.id, { onDelete: "cascade" }).notNull(),
...
var employeeCommissions = pgTable("employee_commissions", {
  id: serial("id").primaryKey(),
  employeeId: integer("employee_id").references(() => employees.id, { onDelete: "cascade" }).notNull(),
  saleId: integer("sale_id").references(() => sales.id, { onDelete: "cascade" }).notNull(),
...
var tenantSubscriptions = pgTable("tenant_subscriptions", {
  id: serial("id").primaryKey(),
...
var tenantNotifications = pgTable("tenant_notifications", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "cascade" }),
  type: text("type").notNull(),
...
var platformCommissions = pgTable("platform_commissions", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
...
var onlineOrders = pgTable("online_orders", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
...
var landingPageConfig = pgTable("landing_page_config", {
  id: serial("id").primaryKey(),
...
var printerConfigs = pgTable("printer_configs", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
  branchId: integer("branch_id").references(() => branches.id, { onDelete: "cascade" }),
...
var dailyClosings = pgTable("daily_closings", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
  branchId: integer("branch_id").references(() => branches.id, { onDelete: "cascade" }),
  employeeId: integer("employee_id").references(() => employees.id, { onDelete: "cascade" }),
...
var monthlyClosings = pgTable("monthly_closings", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
  branchId: integer("branch_id").references(() => branches.id, { onDelete: "cascade" }),
  employeeId: integer("employee_id").references(() => employees.id, { onDelete: "cascade" }),
...
var insertInventorySchema = createInsertSchema(inventory).omit({ id: true, updatedAt: true });
...
var insertPurchaseOrderSchema = createInsertSchema(purchaseOrders).omit({ id: true, createdAt: true });
var insertPurchaseOrderItemSchema = createInsertSchema(purchaseOrderItems).omit({ id: true });
var insertShiftSchema = createInsertSchema(shifts).omit({ id: true });
...
var insertCallSchema = createInsertSchema(calls).omit({ id: true, createdAt: true });
var insertTableSchema = createInsertSchema(tables).omit({ id: true, createdAt: true });
var insertKitchenOrderSchema = createInsertSchema(kitchenOrders).omit({ id: true, createdAt: true, updatedAt: true });
...
var insertActivityLogSchema = createInsertSchema(activityLog).omit({ id: true, createdAt: true });
var insertReturnSchema = createInsertSchema(returns).omit({ id: true, createdAt: true });
var insertReturnItemSchema = createInsertSchema(returnItems).omit({ id: true });
var insertCashDrawerOperationSchema = createInsertSchema(cashDrawerOperations).omit({ id: true, createdAt: true });
var insertWarehouseSchema = createInsertSchema(warehouses).omit({ id: true, createdAt: true });
var insertWarehouseTransferSchema = createInsertSchema(warehouseTransfers).omit({ id: true, createdAt: true });
var insertProductBatchSchema = createInsertSchema(productBatches).omit({ id: true, createdAt: true });
var insertInventoryMovementSchema = createInsertSchema(inventoryMovements).omit({ id: true, createdAt: true });
var insertStockCountSchema = createInsertSchema(stockCounts).omit({ id: true, createdAt: true });
var insertStockCountItemSchema = createInsertSchema(stockCountItems).omit({ id: true });
var insertSupplierContractSchema = createInsertSchema(supplierContracts).omit({ id: true, createdAt: true });
var insertEmployeeCommissionSchema = createInsertSchema(employeeCommissions).omit({ id: true, createdAt: true });
var insertSuperAdminSchema = createInsertSchema(superAdmins).omit({ id: true, createdAt: true, updatedAt: true });
var insertTenantSchema = createInsertSchema(tenants).omit({ id: true, createdAt: true, updatedAt: true });
var insertTenantSubscriptionSchema = createInsertSchema(tenantSubscriptions).omit({ id: true, createdAt: true, updatedAt: true });
var insertLicenseKeySchema = createInsertSchema(licenseKeys).omit({ id: true, createdAt: true, updatedAt: true });
var insertTenantNotificationSchema = createInsertSchema(tenantNotifications).omit({ id: true, createdAt: true });
var insertOnlineOrderSchema = createInsertSchema(onlineOrders).omit({ id: true, createdAt: true, updatedAt: true });
var insertLandingPageConfigSchema = createInsertSchema(landingPageConfig).omit({ id: true, createdAt: true, updatedAt: true });
var insertPlatformSettingSchema = createInsertSchema(platformSettings).omit({ id: true, updatedAt: true });
var insertPlatformCommissionSchema = createInsertSchema(platformCommissions).omit({ id: true, createdAt: true });
var insertVehicleSchema = createInsertSchema(vehicles).omit({ id: true, createdAt: true });
var insertPrinterConfigSchema = createInsertSchema(printerConfigs).omit({ id: true, updatedAt: true });
var insertDailyClosingSchema = createInsertSchema(dailyClosings).omit({ id: true, createdAt: true });
var insertMonthlyClosingSchema = createInsertSchema(monthlyClosings).omit({ id: true, createdAt: true });

// server/db.ts
var { Pool: Pool3 } = esm_default;
var connectionString = process.env.NODE_ENV === "production" ? process.env.DATABASE_URL : process.env.NEON_DATABASE_URL || process.env.DATABASE_URL;
if (!connectionString) {
  throw new Error(
...
`);
  ...
  async getLowStockItems(branchId) {
    const { inArray: inArray2, notInArray: notInArray2, lte: ltEq } = await Promise.resolve().then(() => (init_drizzle_orm(), drizzle_orm_exports));
    ...
    if (excludedBranchIds.length > 0) {
      conditions.push(notInArray2(inventory.branchId, excludedBranchIds));
    }
  ...
  async getProductBatches(productId, tenantId) {
    ...
    return db.select().from(productBatches).where(and2(...conditions)).orderBy(productBatches.expiryDate);
  },
