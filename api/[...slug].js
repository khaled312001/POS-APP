...
__export(schema_exports, {
  ...
  insertTableQrCodeSchema: () => insertTableQrCodeSchema,
  ...
  restaurantAggregateRatings: () => restaurantAggregateRatings,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    tableQrCodes = (0, import_mysql_core.mysqlTable)("table_qr_codes", {
      id: (0, import_mysql_core.serial)("id").primaryKey(),
      tenantId: (0, import_mysql_core.int)("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
      tableId: (0, import_mysql_core.int)("table_id").references(() => tables.id, { onDelete: "cascade" }).notNull(),
      branchId: (0, import_mysql_core.int)("branch_id").references(() => branches.id, { onDelete: "cascade" }),
      qrToken: (0, import_mysql_core.varchar)("qr_token", { length: 64 }).notNull().unique(),
      tableName: (0, import_mysql_core.text)("table_name").notNull(),
      isActive: (0, import_mysql_core.boolean)("is_active").default(true),
      scannedCount: (0, import_mysql_core.int)("scanned_count").default(0),
      lastScannedAt: (0, import_mysql_core.timestamp)("last_scanned_at"),
      createdAt: (0, import_mysql_core.timestamp)("created_at").defaultNow()
    });
    ...
    onlineOrders = (0, import_mysql_core.mysqlTable)("online_orders", {
      ...
      tableNumber: (0, import_mysql_core.text)("table_number"),
      tableQrToken: (0, import_mysql_core.varchar)("table_qr_token", { length: 64 }),
      createdAt: (0, import_mysql_core.timestamp)("created_at").defaultNow(),
      updatedAt: (0, import_mysql_core.timestamp)("updated_at").defaultNow()
    });
    ...
    restaurantAggregateRatings = (0, import_mysql_core.mysqlTable)("restaurant_aggregate_ratings", {
      id: (0, import_mysql_core.serial)("id").primaryKey(),
      tenantId: (0, import_mysql_core.int)("tenant_id").references(() => tenants.id, { onDelete: "cascade" }).notNull(),
      avgRating: (0, import_mysql_core.decimal)("avg_rating", { precision: 3, scale: 2 }).default("0"),
      totalReviews: (0, import_mysql_core.int)("total_reviews").default(0),
      avgDeliveryTime: (0, import_mysql_core.int)("avg_delivery_time").default(30),
      updatedAt: (0, import_mysql_core.timestamp)("updated_at").defaultNow()
    });
    ...
    insertTableQrCodeSchema = (0, import_drizzle_zod.createInsertSchema)(tableQrCodes).omit({ id: true, createdAt: true });
...
`);
      ...
      // Table QR Codes
      async getTableQrCodes(tenantId) {
        return db.select().from(tableQrCodes).where((0, import_drizzle_orm.eq)(tableQrCodes.tenantId, tenantId)).orderBy(tableQrCodes.tableName);
      },
      async getTableQrCodeByToken(token) {
        const [qr] = await db.select().from(tableQrCodes).where((0, import_drizzle_orm.eq)(tableQrCodes.qrToken, token));
        return qr;
      },
      async createTableQrCode(data) {
        const _ins = await db.insert(tableQrCodes).values(data).$returningId();
        const [qr] = await db.select().from(tableQrCodes).where((0, import_drizzle_orm.eq)(tableQrCodes.id, _ins[0]?.id ?? 0));
        return qr;
      },
      async updateTableQrCode(id, data) {
        await db.update(tableQrCodes).set(data).where((0, import_drizzle_orm.eq)(tableQrCodes.id, id));
        const [qr] = await db.select().from(tableQrCodes).where((0, import_drizzle_orm.eq)(tableQrCodes.id, id));
        return qr;
      },
      async deleteTableQrCode(id) {
        await db.delete(tableQrCodes).where((0, import_drizzle_orm.eq)(tableQrCodes.id, id));
      },
      async incrementQrScanCount(token) {
        await db.update(tableQrCodes).set({ scannedCount: import_drizzle_orm.sql`scanned_count + 1`, lastScannedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm.eq)(tableQrCodes.qrToken, token));
      },
      // Kitchen Orders
...
async function seedPizzaLemon() {
  ...
  const [existingKey] = await db.select().from(licenseKeys).where((0, import_drizzle_orm5.eq)(licenseKeys.licenseKey, LICENSE_KEY));
  const isAlreadySeeded = !!existingKey;
  if (isAlreadySeeded) {
    console.log("[PIZZA LEMON] License key already present \u2013 running full catalog update...");
  }
  let tenant;
  const pizzaLemonTenants = await db.select().from(tenants).where((0, import_drizzle_orm5.eq)(tenants.id, 24));
  if (pizzaLemonTenants.length > 0) {
    ...
    await db.update(tenants).set({
      ...
      maxBranches: 3,
      maxEmployees: 20
    }).where((0, import_drizzle_orm5.eq)(tenants.id, 24));
  } else {
  ...
  const subs = await db.select().from(tenantSubscriptions).where((0, import_drizzle_orm5.eq)(tenantSubscriptions.tenantId, tenant.id));
  ...
  let tenantBranches = await db.select().from(branches).where((0, import_drizzle_orm5.eq)(branches.tenantId, tenant.id));
  let branchId;
  if (tenantBranches.length > 0) {
    branchId = tenantBranches[0].id;
  } else {
  ...
  const existingEmps = await db.select().from(employees).where((0, import_drizzle_orm5.eq)(employees.branchId, branchId));
  ...
  const existingProds = await db.select().from(products).where((0, import_drizzle_orm5.eq)(products.tenantId, tenant.id));
  ...
  if (shouldForceCatalogReset) {
    const existingCats = await db.select({ id: categories.id, name: categories.name }).from(categories).where((0, import_drizzle_orm5.eq)(categories.tenantId, tenant.id));
    if (existingProds.length > 0) {
      await db.delete(products).where((0, import_drizzle_orm5.eq)(products.tenantId, tenant.id));
    }
    if (existingCats.length > 0) {
      await db.delete(categories).where((0, import_drizzle_orm5.eq)(categories.tenantId, tenant.id));
    }
    console.log(
  ...
  const allCats = await db.select({ id: categories.id, name: categories.name }).from(categories).where((0, import_drizzle_orm5.eq)(categories.tenantId, tenant.id));
  const catMap = {};
  for (const c of allCats) catMap[c.name] = c.id;
  if (catMap["Softgetr\xE4nke"] && !catMap["Getr\xE4nke"]) {
    await db.update(categories).set({ name: "Getr\xE4nke" }).where((0, import_drizzle_orm5.eq)(categories.id, catMap["Softgetr\xE4nke"]));
  ...
  if (catMap["Tabakwaren"] && !catMap["Extra"]) {
    await db.update(categories).set({ name: "Extra", icon: "add-circle" }).where((0, import_drizzle_orm5.eq)(categories.id, catMap["Tabakwaren"]));
  ...
  for (const cat of PIZZA_LEMON_CATEGORIES) {
    if (!catMap[cat.name]) {
      ...
      await db.update(categories).set({
        ...
        color: cat.color,
        icon: cat.icon
      }).where((0, import_drizzle_orm5.eq)(categories.id, catMap[cat.name]));
    }
  }
  let idx = 0;
  ...
  const [existingConfig] = await db.select().from(landingPageConfig).where((0, import_drizzle_orm5.eq)(landingPageConfig.tenantId, tenant.id));
  ...
  if (!existingConfig) {
    ...
    await db.update(landingPageConfig).set({
      ...
      deliveryRadius: "Zone 1 (ab 20.-): Affoltern, Seebach, Oerlikon | Zone 2 (ab 30.-): Kloten, Wallisellen | Zone 3 (ab 40.-): Regensdorf"
    }).where((0, import_drizzle_orm5.eq)(landingPageConfig.tenantId, tenant.id));
  ...
  const existingVehicles = await db.select().from(vehicles).where((0, import_drizzle_orm5.eq)(vehicles.tenantId, tenant.id));
...
var import_drizzle_orm5, import_bcrypt2, import_date_fns2, STORE_EMAIL, STORE_PASSWORD, LICENSE_KEY, BUSINESS_NAME, IMG, PIZZA_LEMON_CATEGORIES, PIZZAS, CALZONES, PIDE, LAHMACUN, TELLERGERICHTE, FINGERFOOD, SALATE, DESSERTS, GETRAENKE, BIER, ALKOHOL, EXTRAS;
...
var import_drizzle_orm6, crypto4, import_bcrypt3, import_date_fns3, DEMO_STORES, CATEGORY_NAMES, PRODUCT_NAMES, CUSTOMER_NAMES, SUPPLIER_NAMES;
...
var PUBLIC_ROUTES = [
  ...
  "/api/dine-in/validate/",
  // Public QR token validation for dine-in
  "/api/delivery/auth/",
  ...
  "/api/driver",
  // Driver PWA HTML (exact)
  "/api/driver/",
  // Driver PWA HTML (with slash/slug)
  "/api/restaurants",
...
function generateToken(adminId, email, role) {
  return import_jsonwebtoken2.default.sign({ id: adminId, email, role }, JWT_SECRET2, { expiresIn: "24h" });
}
...
var BACKUP_DIR = process.env.VERCEL ? path.join("/tmp", "backups") : path.resolve(process.cwd(), "backups");
try {
  if (!fs2.existsSync(BACKUP_DIR)) fs2.mkdirSync(BACKUP_DIR, { recursive: true });
} catch (err) {
  console.warn("[superAdminRoutes] Could not create BACKUP_DIR:", err.message);
}
async function createTenantBackup(tenantId) {
  ...
  fs2.writeFileSync(filepath, JSON.stringify(snapshot, null, 2));
  console.log(`[BACKUP] Created ${filename} (${Math.round(fs2.statSync(filepath).size / 1024)}KB)`);
  return filename;
}
...
function registerSuperAdminRoutes(app2) {
  ...
  app2.get("/api/super-admin/analytics/overview", requireSuperAdmin, async (_req, res) => {
    try {
      ...
      const [totalTenants] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(tenants2);
      const [activeTenants] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(tenants2).where((0, import_drizzle_orm2.eq)(tenants2.status, "active"));
      const [totalSubs] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(tenantSubscriptions2);
      const [activeSubs] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(tenantSubscriptions2).where((0, import_drizzle_orm2.eq)(tenantSubscriptions2.status, "active"));
      const [totalLicenses] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(licenseKeys2);
      const [activeLicenses] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(licenseKeys2).where((0, import_drizzle_orm2.eq)(licenseKeys2.status, "active"));
      const [totalBranches] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(branches2);
      const [totalEmployees] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(employees2);
      const [totalProducts] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(products2);
      const [totalSales] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(sales2);
      const [totalCustomers] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(customers2);
      const [revenueRow] = await db.select({ total: import_drizzle_orm2.sql`cast(coalesce(sum(cast(price as decimal(10,2))), 0) as char)` }).from(tenantSubscriptions2).where((0, import_drizzle_orm2.eq)(tenantSubscriptions2.status, "active"));
      const [salesRevenue] = await db.select({ total: import_drizzle_orm2.sql`cast(coalesce(sum(cast(total_amount as decimal(12,2))), 0) as char)` }).from(sales2);
      const in7Days = /* @__PURE__ */ new Date();
      in7Days.setDate(in7Days.getDate() + 7);
      const now = /* @__PURE__ */ new Date();
      const [expiringSubs] = await db.select({ count: import_drizzle_orm2.sql`count(*)` }).from(tenantSubscriptions2).where((0, import_drizzle_orm2.and)((0, import_drizzle_orm2.eq)(tenantSubscriptions2.status, "active"), (0, import_drizzle_orm2.lte)(tenantSubscriptions2.endDate, in7Days), (0, import_drizzle_orm2.gte)(tenantSubscriptions2.endDate, now)));
      res.json({
  ...
  app2.post("/api/super-admin/tenants", requireSuperAdmin, async (req, res) => {
    try {
      ...
      const startDate = /* @__PURE__ */ new Date();
      const endDate = (0, import_date_fns.addDays)(startDate, 14);
  ...
  app2.post("/api/super-admin/subscriptions", requireSuperAdmin, async (req, res) => {
    try {
      ...
      if (planType === "monthly") endDate = (0, import_date_fns.addMonths)(startDate, 1);
      else if (planType === "yearly") endDate = (0, import_date_fns.addYears)(startDate, 1);
      else endDate = (0, import_date_fns.addDays)(startDate, 30);
      const sub = await storage.createTenantSubscription({
        tenantId,
  ...
  app2.post("/api/super-admin/subscriptions/:id/extend", requireSuperAdmin, async (req, res) => {
    try {
      ...
      if (months) newEnd = (0, import_date_fns.addMonths)(currentEnd, months);
      else if (days) newEnd = (0, import_date_fns.addDays)(currentEnd, days);
  ...
  app2.post("/api/super-admin/licenses/generate", requireSuperAdmin, async (req, res) => {
    try {
      ...
      const key = await storage.createLicenseKey({
        ...
        expiresAt: expiresAt ? new Date(expiresAt) : (0, import_date_fns.addYears)(/* @__PURE__ */ new Date(), 1),
        notes: notes || null
      });
      res.json(key);
    } catch (e) {
  ...
  app2.delete("/api/super-admin/licenses/:id", requireSuperAdmin, async (req, res) => {
    try {
      ...
      await db.delete(licenseKeys2).where((0, import_drizzle_orm2.eq)(licenseKeys2.id, id));
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  app2.get("/api/super-admin/backup/list", requireSuperAdmin, async (_req, res) => {
    try {
      const files = fs2.readdirSync(BACKUP_DIR).filter((f) => f.endsWith(".json")).map((f) => {
        const stat = fs2.statSync(path.join(BACKUP_DIR, f));
  ...
  app2.get("/api/super-admin/backup/download/:filename", requireSuperAdmin, async (req, res) => {
    try {
      ...
      res.send(fs2.readFileSync(filepath));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  app2.post("/api/super-admin/backup/restore/:filename", requireSuperAdmin, async (req, res) => {
    try {
      const filename = path.basename(req.params.filename);
      const filepath = path.join(BACKUP_DIR, filename);
      if (!fs2.existsSync(filepath)) return res.status(404).json({ error: "Backup file not found" });
  ...
  app2.get("/api/super-admin/expenses", requireSuperAdmin, async (_req, res) => {
    try {
      ...
      const rows = await db.select().from(expenses2).orderBy((0, import_drizzle_orm2.desc)(expenses2.createdAt)).limit(500);
      res.json(rows);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  app2.get("/api/super-admin/shifts/all", requireSuperAdmin, async (_req, res) => {
    try {
      ...
      const rows = await db.select().from(shifts2).orderBy((0, import_drizzle_orm2.desc)(shifts2.startTime)).limit(200);
      res.json(rows);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  app2.post("/api/super-admin/db-migrate-to-replit-neon", requireSuperAdmin, async (_req, res) => {
    ...
    try {
      ...
      log2.push(`ERROR: ${e.message}`);
      res.status(500).json({ success: false, log: log2, error: e.message });
    }
  });
...
var import_storage4 = require("@google-cloud/storage");
var import_crypto = require("crypto");
var REPLIT_SIDECAR_ENDPOINT = "http://127.0.0.1:1106";
var objectStorageClient = new import_storage4.Storage({
  credentials: {
    audience: "replit",
...
async function createOtp(phone, tenantId) {
  ...
  await db.update(otpVerifications).set({ verified: true }).where(
    (0, import_drizzle_orm3.and)(
      (0, import_drizzle_orm3.eq)(otpVerifications.phone, phone),
      (0, import_drizzle_orm3.eq)(otpVerifications.tenantId, tenantId),
      (0, import_drizzle_orm3.eq)(otpVerifications.verified, false)
    )
  );
...
async function verifyOtp(phone, tenantId, inputOtp) {
  ...
  const [record] = await db.select().from(otpVerifications).where(
    (0, import_drizzle_orm3.and)(
      ...
      (0, import_drizzle_orm3.eq)(otpVerifications.tenantId, tenantId),
      (0, import_drizzle_orm3.eq)(otpVerifications.verified, false),
      (0, import_drizzle_orm3.gt)(otpVerifications.expiresAt, now)
    )
  ...
  if (attempts > OTP_MAX_ATTEMPTS) {
    await db.update(otpVerifications).set({ verified: true }).where((0, import_drizzle_orm3.eq)(otpVerifications.id, record.id));
    return { success: false, error: "Too many attempts. Please request a new OTP." };
  }
  await db.update(otpVerifications).set({ attempts }).where((0, import_drizzle_orm3.eq)(otpVerifications.id, record.id));
...
async function findOrCreateCustomerByPhone(phone, tenantId) {
  const [existing] = await db.select().from(customers).where((0, import_drizzle_orm3.and)((0, import_drizzle_orm3.eq)(customers.phone, phone), (0, import_drizzle_orm3.eq)(customers.tenantId, tenantId))).limit(1);
  ...
  const [newCustomer] = await db.select().from(customers).where((0, import_drizzle_orm3.eq)(customers.id, inserted.id)).limit(1);
  return newCustomer;
}
async function findCustomerByEmail(email, tenantId) {
  const [customer] = await db.select().from(customers).where((0, import_drizzle_orm3.and)((0, import_drizzle_orm3.eq)(customers.email, email), (0, import_drizzle_orm3.eq)(customers.tenantId, tenantId))).limit(1);
  return customer ?? null;
}
...
async function setCustomerPassword(customerId, password) {
  ...
  await db.update(customers).set({ passwordHash: hash3, hasAccount: true }).where((0, import_drizzle_orm3.eq)(customers.id, customerId));
}
...
async function getCustomerBySession(token) {
  const now = /* @__PURE__ */ new Date();
  const [session] = await db.select().from(customerSessions).where((0, import_drizzle_orm3.and)((0, import_drizzle_orm3.eq)(customerSessions.token, token), (0, import_drizzle_orm3.gt)(customerSessions.expiresAt, now))).limit(1);
  if (!session) return null;
  const [customer] = await db.select().from(customers).where((0, import_drizzle_orm3.eq)(customers.id, session.customerId)).limit(1);
  return customer ?? null;
}
async function deleteCustomerSession(token) {
  await db.delete(customerSessions).where((0, import_drizzle_orm3.eq)(customerSessions.token, token));
}
...
async function validatePromoCode(tenantId, code, orderTotal, orderType, customerId) {
  ...
  const [promo] = await db.select().from(promoCodes).where(
    (0, import_drizzle_orm4.and)(
      (0, import_drizzle_orm4.eq)(promoCodes.tenantId, tenantId),
      (0, import_drizzle_orm4.eq)(promoCodes.code, code.toUpperCase()),
      (0, import_drizzle_orm4.eq)(promoCodes.isActive, true)
    )
  ).limit(1);
  ...
  if (customerId && promo.perCustomerLimit) {
    const [usageCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(promoCodeUsages).where(
      (0, import_drizzle_orm4.and)(
        (0, import_drizzle_orm4.eq)(promoCodeUsages.promoCodeId, promo.id),
        (0, import_drizzle_orm4.eq)(promoCodeUsages.customerId, customerId)
      )
    );
...
async function recordPromoUsage(promoCodeId, customerId, orderId, discountApplied) {
  ...
  await db.update(promoCodes).set({ usageCount: import_drizzle_orm4.sql`usage_count + 1` }).where((0, import_drizzle_orm4.eq)(promoCodes.id, promoCodeId));
}
async function getLoyaltyConfig(tenantId) {
  ...
  }).from(landingPageConfig).where((0, import_drizzle_orm4.eq)(landingPageConfig.tenantId, tenantId)).limit(1);
  return {
...
async function awardLoyaltyPoints(customerId, tenantId, orderId, orderTotal) {
  ...
  const [customer] = await db.select({ loyaltyPoints: customers.loyaltyPoints }).from(customers).where((0, import_drizzle_orm4.eq)(customers.id, customerId)).limit(1);
  const before = customer?.loyaltyPoints ?? 0;
  const after = before + pointsToAdd;
  const newTier = calculateLoyaltyTier(after);
  await db.update(customers).set({ loyaltyPoints: after, loyaltyTier: newTier }).where((0, import_drizzle_orm4.eq)(customers.id, customerId));
...
async function assignDriverToOrder(orderId, vehicleId) {
  await db.update(onlineOrders).set({ driverId: vehicleId }).where((0, import_drizzle_orm4.eq)(onlineOrders.id, orderId));
  await db.update(vehicles).set({ driverStatus: "on_delivery", activeOrderId: orderId }).where((0, import_drizzle_orm4.eq)(vehicles.id, vehicleId));
}
async function releaseDriver(vehicleId) {
  await db.update(vehicles).set({ driverStatus: "available", activeOrderId: null }).where((0, import_drizzle_orm4.eq)(vehicles.id, vehicleId));
}
async function deductWallet(customerId, tenantId, amount, orderId) {
  const [customer] = await db.select({ walletBalance: customers.walletBalance }).from(customers).where((0, import_drizzle_orm4.eq)(customers.id, customerId)).limit(1);
  const balance = parseFloat(customer?.walletBalance ?? "0");
  if (amount > balance)
    return { success: false, error: "Insufficient wallet balance" };
  const after = Math.round((balance - amount) * 100) / 100;
  await db.update(customers).set({ walletBalance: after.toFixed(2) }).where((0, import_drizzle_orm4.eq)(customers.id, customerId));
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/table-qr-codes", async (req, res) => {
    try {
      const tenantId = Number(req.query.tenantId);
  ...
  app2.post("/api/table-qr-codes", async (req, res) => {
    try {
  ...
  app2.post("/api/table-qr-codes/generate-all", async (req, res) => {
    try {
      ...
      const existingTableIds = new Set(existing.map((q) => q.tableId));
      const created = [];
  ...
  app2.put("/api/table-qr-codes/:id", async (req, res) => {
    try {
      res.json(await storage.updateTableQrCode(Number(req.params.id), sanitizeDates(req.body)));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  app2.delete("/api/table-qr-codes/:id", async (req, res) => {
    try {
      await storage.deleteTableQrCode(Number(req.params.id));
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  app2.get("/api/dine-in/validate/:token", async (req, res) => {
    try {
...
`;
  ...
  const TENANT_BACKUP_DIR = process.env.VERCEL ? import_node_path.default.join("/tmp", "backups") : import_node_path.default.resolve(process.cwd(), "backups");
  try {
    if (!import_node_fs.default.existsSync(TENANT_BACKUP_DIR)) import_node_fs.default.mkdirSync(TENANT_BACKUP_DIR, { recursive: true });
  } catch (err) {
    console.warn("[routes] Could not create TENANT_BACKUP_DIR:", err.message);
  }
...
Valid for 10 minutes.`);
    ...
    try {
      ...
      for (const r of restaurants) {
        if (r.name === "Restaurant" && r.slug) {
          r.name = r.slug.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
        }
        if (!r.cuisine || !r.coverImage || !r.reviewCount) {
          try {
            const cats = await storage.getCategories(r.id);
            if (!r.cuisine && cats.length > 0) {
              const catNames = cats.filter((c) => c.isActive !== false).map((c) => c.name).slice(0, 3);
  ...
  async function resolveSlugConfig(slug) {
    ...
    if (config) {
      ...
      config.storeName = displayName;
      config.heroTitle = displayName;
    }
    return config;
  }
  ...
  app2.get("/api/delivery/store/:slug/menu", async (req, res) => {
    try {
      ...
      const activeProds = prods.filter((p) => p.isActive !== false).map((p) => ({ ...p, imageUrl: p.imageUrl || p.image || null }));
  ...
  app2.get("/api/delivery/store/:slug/product/:id", async (req, res) => {
    try {
      ...
      res.json({ ...product, imageUrl: product.imageUrl || product.image || null });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
...
`;
  ...
  app2.get("/api/delivery/store/:slug/reviews", async (req, res) => {
    try {
      ...
      for (const r of ratings) {
        ...
        sum2 += r.overallRating || r.foodRating || 0;
      }
      const avgRating = total > 0 ? sum2 / total : 0;
  ...
  const _recoCache = /* @__PURE__ */ new Map();
  const RECO_TTL = 5 * 60 * 1e3;
  app2.get("/api/delivery/recommendations", async (req, res) => {
    try {
      const tid = parseInt(req.query.tenantId) || req.tenantId;
      ...
      const products2 = await storage.getProductsByTenant(tid);
      const popular = products2.filter((p) => p.isActive !== false && parseFloat(p.price || "0") > 0).sort((a, b) => (b.salesCount || 0) - (a.salesCount || 0)).slice(0, 10).map((p) => ({
        id: p.id,
        name: p.name,
        nameAr: p.nameAr,
        price: p.price,
  ...
  app2.get("/api/robots.txt", (_req, res) => {
...
if (!process.env.DATABASE_URL) {
  ...
  if (host) {
    process.env.DATABASE_URL = `postgresql://${process.env.PGUSER}:${process.env.PGPASSWORD}@${host}:${process.env.PGPORT || 5432}/${process.env.PGDATABASE || "neondb"}?sslmode=require`;
...
var ready = (async () => {
  ...
  app.use((err, _req, res, _next) => {
    ...
    if (!res.headersSent) res.status(status2).json({ message });
  });
})().catch((err) => {
