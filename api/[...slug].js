...
async function seedPizzaLemon() {
  ...
  const [existingKey] = await db.select().from(licenseKeys).where((0, import_drizzle_orm3.eq)(licenseKeys.licenseKey, LICENSE_KEY));
  const isAlreadySeeded = !!existingKey;
  if (isAlreadySeeded) {
    console.log("[PIZZA LEMON] License key already present \u2013 running full catalog update...");
  }
  let tenant;
  const pizzaLemonTenants = await db.select().from(tenants).where((0, import_drizzle_orm3.eq)(tenants.id, 24));
  if (pizzaLemonTenants.length > 0) {
    ...
    await db.update(tenants).set({
      ...
      maxBranches: 3,
      maxEmployees: 20
    }).where((0, import_drizzle_orm3.eq)(tenants.id, 24));
  } else {
  ...
  const subs = await db.select().from(tenantSubscriptions).where((0, import_drizzle_orm3.eq)(tenantSubscriptions.tenantId, tenant.id));
  ...
  let tenantBranches = await db.select().from(branches).where((0, import_drizzle_orm3.eq)(branches.tenantId, tenant.id));
  let branchId;
  if (tenantBranches.length > 0) {
    branchId = tenantBranches[0].id;
  } else {
  ...
  const existingEmps = await db.select().from(employees).where((0, import_drizzle_orm3.eq)(employees.branchId, branchId));
  ...
  const existingProds = await db.select().from(products).where((0, import_drizzle_orm3.eq)(products.tenantId, tenant.id));
  ...
  if (shouldForceCatalogReset) {
    const existingCats = await db.select({ id: categories.id, name: categories.name }).from(categories).where((0, import_drizzle_orm3.eq)(categories.tenantId, tenant.id));
    if (existingProds.length > 0) {
      await db.delete(products).where((0, import_drizzle_orm3.eq)(products.tenantId, tenant.id));
    }
    if (existingCats.length > 0) {
      await db.delete(categories).where((0, import_drizzle_orm3.eq)(categories.tenantId, tenant.id));
    }
    console.log(
  ...
  const allCats = await db.select({ id: categories.id, name: categories.name }).from(categories).where((0, import_drizzle_orm3.eq)(categories.tenantId, tenant.id));
  const catMap = {};
  for (const c of allCats) catMap[c.name] = c.id;
  if (catMap["Softgetr\xE4nke"] && !catMap["Getr\xE4nke"]) {
    await db.update(categories).set({ name: "Getr\xE4nke" }).where((0, import_drizzle_orm3.eq)(categories.id, catMap["Softgetr\xE4nke"]));
  ...
  if (catMap["Tabakwaren"] && !catMap["Extra"]) {
    await db.update(categories).set({ name: "Extra", icon: "add-circle" }).where((0, import_drizzle_orm3.eq)(categories.id, catMap["Tabakwaren"]));
  ...
  for (const cat of PIZZA_LEMON_CATEGORIES) {
    if (!catMap[cat.name]) {
      ...
      await db.update(categories).set({
        ...
        color: cat.color,
        icon: cat.icon
      }).where((0, import_drizzle_orm3.eq)(categories.id, catMap[cat.name]));
    }
  }
  let idx = 0;
  ...
  const [existingConfig] = await db.select().from(landingPageConfig).where((0, import_drizzle_orm3.eq)(landingPageConfig.tenantId, tenant.id));
  ...
  if (!existingConfig) {
    ...
    await db.update(landingPageConfig).set({
      ...
      deliveryRadius: "Zone 1 (ab 20.-): Affoltern, Seebach, Oerlikon | Zone 2 (ab 30.-): Kloten, Wallisellen | Zone 3 (ab 40.-): Regensdorf"
    }).where((0, import_drizzle_orm3.eq)(landingPageConfig.tenantId, tenant.id));
  ...
  const existingVehicles = await db.select().from(vehicles).where((0, import_drizzle_orm3.eq)(vehicles.tenantId, tenant.id));
...
var import_drizzle_orm3, import_bcrypt, import_date_fns2, STORE_EMAIL, STORE_PASSWORD, LICENSE_KEY, BUSINESS_NAME, IMG, PIZZA_LEMON_CATEGORIES, PIZZAS, CALZONES, PIDE, LAHMACUN, TELLERGERICHTE, FINGERFOOD, SALATE, DESSERTS, GETRAENKE, BIER, ALKOHOL, EXTRAS;
...
function uuid() {
  return crypto2.randomUUID().split("-")[0].toUpperCase();
}
async function seedAllDemoData() {
  console.log("[SEED] Starting comprehensive demo data seeding...");
  const [saCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(superAdmins);
  ...
  const [tenantCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(tenants);
  ...
  for (const t of allTenants) {
    ...
    let tenantBranches = await db.select().from(branches).where((0, import_drizzle_orm4.eq)(branches.tenantId, t.id));
    if (tenantBranches.length === 0) {
      ...
      tenantBranches = await db.select().from(branches).where((0, import_drizzle_orm4.eq)(branches.tenantId, t.id));
    }
    const branchIds = tenantBranches.map((b) => b.id);
    if (branchIds.length === 0) continue;
    for (const bId of branchIds) {
      const [existingWH] = await db.select().from(warehouses).where((0, import_drizzle_orm4.eq)(warehouses.branchId, bId)).limit(1);
      if (!existingWH) {
    ...
    let tenantEmployees = await db.select().from(employees).where(import_drizzle_orm4.sql`${employees.branchId} IN (${import_drizzle_orm4.sql.join(branchIds, import_drizzle_orm4.sql`, `)})`);
    if (tenantEmployees.length === 0) {
      ...
      tenantEmployees = await db.select().from(employees).where(import_drizzle_orm4.sql`${employees.branchId} IN (${import_drizzle_orm4.sql.join(branchIds, import_drizzle_orm4.sql`, `)})`);
    }
    const employeeIds = tenantEmployees.map((e) => e.id);
    let tenantProducts = await db.select().from(products).where((0, import_drizzle_orm4.eq)(products.tenantId, t.id));
    if (tenantProducts.length === 0) {
      ...
      tenantProducts = await db.select().from(products).where((0, import_drizzle_orm4.eq)(products.tenantId, t.id));
    }
    const productIds = tenantProducts.map((p) => p.id);
    for (const pId of productIds) {
      for (const bId of branchIds) {
        const [existingInv] = await db.select().from(inventory).where(import_drizzle_orm4.sql`${inventory.productId} = ${pId} AND ${inventory.branchId} = ${bId}`);
        if (!existingInv) {
          await db.insert(inventory).values({
            productId: pId,
            branchId: bId,
    ...
    const [custCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(customers);
    ...
    const [supCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(suppliers);
    ...
    const [saleCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(sales).where(import_drizzle_orm4.sql`${sales.branchId} IN (${import_drizzle_orm4.sql.join(branchIds, import_drizzle_orm4.sql`, `)})`);
    ...
    for (const eId of employeeIds) {
      const [existingShift] = await db.select().from(shifts).where((0, import_drizzle_orm4.eq)(shifts.employeeId, eId)).limit(1);
      if (!existingShift) {
        const bId = pick(branchIds);
    ...
    for (const bId of branchIds) {
      const [existingTable] = await db.select().from(tables).where((0, import_drizzle_orm4.eq)(tables.branchId, bId)).limit(1);
      if (!existingTable) {
        for (let i = 1; i <= 5; i++) {
    ...
    const [expCount] = await db.select({ count: import_drizzle_orm4.sql`count(*)` }).from(expenses).where(import_drizzle_orm4.sql`${expenses.branchId} IN (${import_drizzle_orm4.sql.join(branchIds, import_drizzle_orm4.sql`, `)})`);
    ...
    for (const pId of productIds) {
      const [existingBatch] = await db.select().from(productBatches).where((0, import_drizzle_orm4.eq)(productBatches.productId, pId)).limit(1);
      if (!existingBatch) {
        const bId = pick(branchIds);
    ...
    for (const bId of branchIds) {
      const [existingSC] = await db.select().from(stockCounts).where((0, import_drizzle_orm4.eq)(stockCounts.branchId, bId)).limit(1);
      if (!existingSC) {
...
var import_drizzle_orm4, crypto2, import_bcrypt2, import_date_fns3, DEMO_STORES, CATEGORY_NAMES, PRODUCT_NAMES, CUSTOMER_NAMES, SUPPLIER_NAMES;
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/landing/confirm-subscription", async (req, res) => {
    try {
      ...
      const randomSegments = Array.from({ length: 4 }, () => crypto3.randomBytes(2).toString("hex").toUpperCase());
      const licenseKey = `BARMAGLY-${randomSegments.join("-")}`;
      await storage.createLicenseKey({
        licenseKey,
        tenantId: tenant.id,
  ...
  app2.post("/api/auth/google", async (req, res) => {
    try {
      ...
      if (!tenant) {
        isNew = true;
        const tempPassword = "GAuth-" + crypto3.randomBytes(4).toString("hex");
        const passwordHash = await bcrypt4.hash(tempPassword, 10);
